...
- **Navidrome integration (optional)** – downloaded playlists can be made
  public or assigned to a specific Navidrome user, preselected to whoever is
  signed in, and no download overwrites a playlist that belongs to somebody
  else: Spotify hands every account its own "Daily Mix 3", so the second one
  is filed as `Daily Mix 3 (2)` and still shows up under its real title
